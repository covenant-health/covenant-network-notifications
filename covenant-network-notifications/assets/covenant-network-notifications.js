...
(function ($) {
  ...
  } // In which we attempt to get MedChat to play nice with GTM


  $(document).ready(function () {
    // Capture the MedChat button click to expand the chat
    $('#medchat-outreach button:first-of-type, #medchat-trigger .mc-expand').click(function () {
      console.log('Chat widget expanded');
      dataLayer.push({
        'event': 'GAEvent',
        'eventCategory': 'MedChat',
        'eventAction': 'Chat Event',
        'eventLabel': 'Chat widget expanded'
      });
    }); // Capture the MedChat button click to close the chat

    $('.wrapper .brand-button.minimize-arrow').click(function () {
      console.log('Chat widget closed');
      dataLayer.push({
        'event': 'GAEvent',
        'eventCategory': 'MedChat',
        'eventAction': 'Chat Event',
        'eventLabel': 'Chat widget closed'
      });
    });

    medchatapp.onChatStart = function () {
      setTimeout(function () {
        console.log('Chat session begun');
        dataLayer.push({
          'event': 'GAEvent',
          'eventCategory': 'MedChat',
          'eventAction': 'Chat Event',
          'eventLabel': 'Chat session begun'
        });
      }, 1000);
    };

    medchatapp.onTopicChanged = function () {
      setTimeout(function () {
        console.log('chat topic changed');
        dataLayer.push({
          'event': 'GAEvent',
          'eventCategory': 'MedChat',
          'eventAction': 'Chat Event',
          'eventLabel': 'Chat topic changed'
        });
      }, 500);
    };

    medchatapp.onChatEnd = function () {
      setTimeout(function () {
